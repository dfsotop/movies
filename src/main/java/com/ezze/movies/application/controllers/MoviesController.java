...
public class MoviesController {
    ...
    public ResponseEntity<List<Movie>> getMoviesByFilter(
            @RequestParam(required = false, name = "metrics") String metricsName,
            @RequestParam(required = false, name = "rating") Integer rating) {
        try {
            if ((metricsName != null && rating != null) || (metricsName == null && rating == null)) {
                return ResponseEntity.badRequest()
                        .contentType(MediaType.APPLICATION_JSON).build();
            }
            if (metricsName != null) {
                return ResponseEntity.ok(moviesService.getByMetrics(metricsName));
            } else {
